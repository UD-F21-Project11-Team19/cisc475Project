...
class Polygon {
  ...
  Polygon(List<Point<num>> points, {this.name = "unknown"})
...
//    name = tname;
  ...
  bool isPointInsideT(Point i, double T) {
    if (isPointInside(i)) {
      ...
      double dis = 0, dd = 0;
      dis = distanceInMeter(i.x.toDouble(), i.y.toDouble(),
          points[0].x.toDouble(), points[0].y.toDouble());
      for (int j = 1; j < points.length; j++) {
        dd = distanceInMeter(i.x.toDouble(), i.y.toDouble(),
            points[j].x.toDouble(), points[j].y.toDouble());
        dis = dd < dis ? dd : dis;
      }
